        ...
        {searchModalIsOpen && search.length > 0 && (
          <ul className="hidden top-[105px] left-0 rounded-b sm:block sm:!absolute sm:top-[51px] w-full max-h-96 shadow-md text-base-content overflow-y-auto bg-[#e5e7eb] text-[#777]">
            {searchedWebtoons.map((webtoon) => {
              return (
                <li onMouseDown={(e) => e.preventDefault()} onClick={() => navigate(`?titleID=${webtoon.webtoonId}`)} className="text-sm bg-white hover:bg-[#eef1f5] hover:text-[#333] transition-all" key={webtoon.webtoonId}>
                  <div className="flex items-center gap-2 p-[12px_16px] cursor-pointer">
