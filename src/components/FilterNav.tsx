      ...
      <div className="flex flex-col gap-3 whitespace-nowrap w-full mb-6 sm:text-base">
        ...
        {weekIsOpen && (
          <ul className="flex justify-between gap-4 w-full sm:w-[428px] md:w-[490px] ">
            <li className={searchQuery.week === 'all' ? 'p-[0.3rem_0.55rem] bg-[#f5f5f5] rounded-full cursor-pointer hidden sm:block' : 'p-[0.3rem_0.55rem] hidden sm:block cursor-pointer'} onClick={() => navigate(`/?week=all`)}>
              요일전체
            </li>
            {weeks.map((week, index) => {
              return (
                <li onClick={() => navigate(`/?week=${Week[week]}`)} key={index} className={searchQuery.week == undefined ? (week === today ? 'p-[0.3rem_0.55rem] bg-[#f5f5f5] rounded-full cursor-pointer' : 'p-[0.3rem_0.55rem] cursor-pointer') : searchQuery.week === Week[week] ? 'p-[0.3rem_0.55rem] bg-[#f5f5f5] rounded-full cursor-pointer' : 'p-[0.3rem_0.55rem] cursor-pointer'}>
                  {week}
                </li>
              )
            })}
